...
export class PlanComponent implements OnInit, AfterViewInit {
  ...
  handleClick(ev: MouseEvent | PointerEvent): void {
    ...
    if(ta && (this.onPrenCreation || this.action == 'mod-posti' || this.action == 'mod-pren-posti')) {
      for (let i = 0; i < (ta.orientamento == 1 ? ta.row : 2); i++)
        for (let j = 0; j < (ta.orientamento == 0 ? ta.row : 2); j++) {
          ...
          if (ev.clientX >= xs && ev.clientX <= xs + this.GAP_PUNTI && ev.clientY >= ys && ev.clientY <= ys + this.GAP_PUNTI) {
            let t = [];
      ...
      let posto = this.getPostoClicked(ev.clientX, ev.clientY, ta);
      if(posto)
    ...
    this.__handleClick(ev.clientX, ev.clientY);
  }

  handleMove(ev: MouseEvent | PointerEvent): void {
    this.cursorClass = 'auto';

    if(this.onPlanMoving) {
      let dx = (ev.clientX - this.prevX) / this.zoom;
      let dy = (ev.clientY - this.prevY) / this.zoom
      this.ref.x += dx;
      this.ref.y += dy;
    } else if(this.onTableMoving) {
      this.coordsTable.x += (ev.clientX - this.prevX) / this.zoom;
      this.coordsTable.y += (ev.clientY - this.prevY) / this.zoom;
    } else if(this.getTableClicked(ev.clientX, ev.clientY))
  ...
  __startTempCLickedTable(X: number, Y: number, evType: 'mouse' | 'touch'): void {
    ...
    if(evType == 'mouse')
      this.timeout = setTimeout(doAction, 400);
    else {
      doAction();
  ...
  startTempCLickedTable(ev: MouseEvent): void {
    this.__startTempCLickedTable(ev.clientX, ev.clientY, 'mouse');
  }
  ...
  modPrenotazione(): void {
    ...
    else if(this.deltaPosti() < 0 || (this.deltaPosti() > 0 && this.inputsPrenotazione.disposizione == 0))
